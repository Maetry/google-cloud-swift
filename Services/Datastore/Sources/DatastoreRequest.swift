...
struct GoogleCloudDatastoreRequest: GoogleCloudAPIClient, Sendable {
    ...
    func send<T: Codable>(method: HTTPMethod,
        ...
        if T.self is GoogleCloudDatastoreDataResponse.Type {
            return GoogleCloudDatastoreDataResponse(data: Data(buffer: responseData)) as! T
        } else {
