...
public protocol SubscriptionsAPI: Sendable {
    ...
    ///
    /// - Parameters:
    ///   - subscriptionId: The name of the subscription to get
    ...
    func get(subscriptionId: String, subscriptionProject: String?) async throws -> GoogleCloudPubSubSubscription
    
    /// Acknowledges the messages associated with the ackIds in the AcknowledgeRequest.
    ///
    /// - Parameters:
    ///   - subscriptionId: ID of the subscription whose message is being acknowledged
    ///   - subscriptionProject: Name of the project that owns the subscription. If not provided, the default project will be used.
    ...
    func acknowledge(subscriptionId: String, subscriptionProject: String?, ackIds: [String]) async throws -> EmptyResponse
    
    /// Creates a subscription to a given topic.
    /// - Parameters:
    ///   - subscriptionId: The name of the subscription to be created.
    ///   - subscriptionProject: Name of the project that owns the subscription. If not provided, the default project will be used.
    ...
    func create(subscriptionId: String,
                ...
                detached: Bool?) async throws -> GoogleCloudPubSubSubscription
}

public extension SubscriptionsAPI {
    func get(subscriptionId: String, subscriptionProject: String? = nil) async throws -> GoogleCloudPubSubSubscription {
        try await get(subscriptionId: subscriptionId, subscriptionProject: subscriptionProject)
    }
    
    func acknowledge(subscriptionId: String, subscriptionProject: String? = nil, ackIds: [String]) async throws -> EmptyResponse {
        try await acknowledge(subscriptionId: subscriptionId, subscriptionProject: subscriptionProject, ackIds: ackIds)
    }
}
...
public final class GoogleCloudPubSubSubscriptionsAPI: SubscriptionsAPI {
    ...
    public func get(subscriptionId: String, subscriptionProject: String? = nil) async throws -> GoogleCloudPubSubSubscription {
        try await request.send(method: .GET, path: "\(endpoint)/v1/projects/\(subscriptionProject ?? request.project)/subscriptions/\(subscriptionId)")
    }
    
    public func acknowledge(subscriptionId: String, subscriptionProject: String? = nil, ackIds: [String]) async throws -> EmptyResponse {
        let acks = AcknowledgeRequest(ackIds: ackIds)
        let body = try HTTPClientRequest.Body.bytes(.init(data: encoder.encode(acks)))
    ...
    public func create(subscriptionId: String,
        ...
        let subscription = GoogleCloudPubSubSubscription(name: subscriptionId,
                                                         ...
                                                         detached: detached)
        let body = try HTTPClientRequest.Body.bytes(.init(data: encoder.encode(subscription)))
